import { LENGTH_RE, LINE_STYLE_KEYWORDS, LINE_WIDTH_KEYWORDS, STYLE_PROP_DEFS } from './config'
...
export function getAtomicKeySet(): Set<string> {
  if (!ATOMIC_KEY_SET) {
    ...
    for (const [base, cfg] of Object.entries(STYLE_PROP_DEFS) as [StyleBase, StyleConfig][]) {
      if (cfg.sides) {
