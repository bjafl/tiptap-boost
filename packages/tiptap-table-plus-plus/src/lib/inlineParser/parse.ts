...
const PATTERN_FIRST_DECL = /(^[\s;]*([a-z\-]+):(.+?))(?:(?:;[\s;]*([a-z\-]+:))|;[;\s]*|\s*$)/
 ...
 */
export function parseDeclarations(cssText: string, ignoreImportant: boolean = true): RawTuple[] {
 ...
 */
export function parseBorderShorthand(value: string): Record<StylePart<'border'>, string> {
