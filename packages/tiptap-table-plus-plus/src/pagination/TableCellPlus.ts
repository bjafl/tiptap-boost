...
function applyCellAttrs(dom: HTMLTableCellElement, node: ProsemirrorNode): void {
  const {
    colspan,
    rowspan,
    backgroundColor,
  ...
  const preAssign = Object.entries(dom.style).filter(([key]) => key.startsWith('border'))
  Object.assign(dom.style, {
    backgroundColor,
    padding,
    textAlign,
    verticalAlign,
    borderTop: borderTop === BORDER_COLLAPSED_SENTINEL ? BORDER_COLLAPSE_STYLE : borderTop,
    borderRight: borderRight === BORDER_COLLAPSED_SENTINEL ? BORDER_COLLAPSE_STYLE : borderRight,
    borderBottom: borderBottom === BORDER_COLLAPSED_SENTINEL ? BORDER_COLLAPSE_STYLE : borderBottom,
    borderLeft: borderLeft === BORDER_COLLAPSED_SENTINEL ? BORDER_COLLAPSE_STYLE : borderLeft,
  } as Record<string, string | null | undefined>)
  console.log('dom assign result:', {
    preAssign,
    postAssign: Object.entries(dom.style).filter(([key]) => key.startsWith('border')),
