...
export type Cap<S extends string> = S extends `${infer F}${infer R}`
  ...
  : S
export type StyleSide<K extends StyleBase> = (typeof STYLE_PROP_DEFS)[K]['sides'] extends true
  ? Side
  : undefined
export type StylePart<K extends StyleBase> =
  (typeof STYLE_PROP_DEFS)[K]['parts'] extends readonly []
    ? undefined
    : (typeof STYLE_PROP_DEFS)[K]['parts'] extends readonly (infer P extends string)[]
      ? Cap<P>
      : undefined
